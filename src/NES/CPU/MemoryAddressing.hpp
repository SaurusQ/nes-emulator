...
using AddressingMode = ::CPU::AddressingMode;
    ...
    struct MemoryAddressing
    {
        inline static bool fetch(CPU& cpu, uint16_t& targetAddress, uint8_t& value, bool& pageCrossed)
        {
            ...
            switch (AM)
            {
                ...
                case IMMEDIATE:
                    cpu.mapper_.read<AS::CPU>(cpu.registers_.PC, value);
                    cpu.registers_.PC++;
                    return false;
    
                case ZERO_PAGE:
                    cpu.mapper_.read<AS::CPU>(cpu.registers_.PC, byte);
                    cpu.registers_.PC++;
                    targetAddress = byte;
                    cpu.mapper_.read<AS::CPU>(targetAddress, value);
                    return true;
                ...
                case ABSOLUTE:
                    cpu.mapper_.read<AS::CPU>(cpu.registers_.PC, low);
                    cpu.registers_.PC++;
                    cpu.mapper_.read<AS::CPU>(cpu.registers_.PC, high);
                    cpu.registers_.PC++;
                ...
                case INDIRECT: // Only JMP supports
                    ...
                    cpu.registers_.PC++;
                    targetAddress = make16(low, high);
                    cpu.mapper_.read<AS::CPU>(targetAddress, low);
                    cpu.mapper_.read<AS::CPU>((targetAddress & 0xFF00) + ((targetAddress + 1) & 0x00FF), high);
                    targetAddress = make16(low, high);
                    return false;
                ...
                case INDIRECT_X:
                    ...
                    targetAddress = byte;
                    cpu.mapper_.read<AS::CPU>(targetAddress,              low);
                    cpu.mapper_.read<AS::CPU>((targetAddress + 1) & 0xFF, high);
                    targetAddress = make16(low, high);
                    cpu.mapper_.read<AS::CPU>(targetAddress, value);
                    return true;
