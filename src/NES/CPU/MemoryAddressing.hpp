    ...
    struct MemoryAddressing
    {
        inline static bool fetch(CPU& cpu, uint16_t& targetAddress, uint8_t& value, bool& pageCrossed)
        {
            ...
            switch (AM)
            {
                ...
                case IMMEDIATE:
                    cpu.bus_.readCPU(cpu.registers_.PC, value);
                    cpu.registers_.PC++;
                    return false;
    
                case ZERO_PAGE:
                    cpu.bus_.readCPU(cpu.registers_.PC, byte);
                    cpu.registers_.PC++;
                    targetAddress = byte;
                    cpu.bus_.readCPU(targetAddress, value);
                    return true;
                ...
                case ABSOLUTE:
                    cpu.bus_.readCPU(cpu.registers_.PC, low);
                    cpu.registers_.PC++;
                    cpu.bus_.readCPU(cpu.registers_.PC, high);
                    cpu.registers_.PC++;
                ...
                case INDIRECT: // Only JMP supports
                    ...
                    cpu.registers_.PC++;
                    targetAddress = make16(low, high);
                    cpu.bus_.readCPU(targetAddress, low);
                    cpu.bus_.readCPU((targetAddress & 0xFF00) + ((targetAddress + 1) & 0x00FF), high);
                    targetAddress = make16(low, high);
                    return false;
                ...
                case INDIRECT_X:
                    ...
                    targetAddress = byte;
                    cpu.bus_.readCPU(targetAddress,              low);
                    cpu.bus_.readCPU((targetAddress + 1) & 0xFF, high);
                    targetAddress = make16(low, high);
                    cpu.bus_.readCPU(targetAddress, value);
                    return true;
